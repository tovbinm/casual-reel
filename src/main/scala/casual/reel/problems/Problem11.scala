...
class Grid(rowz: List[String], productLen: Int) {
  ...
  def allValidProducts =
    for {
      i <- 0 until rows.length
      j <- 0 until rows.length
    } yield validProducts(Position(i,j))

}

class Problem11(grid: Grid) {

  def solve = grid.allValidProducts.flatten.map(grid.productValue).max
